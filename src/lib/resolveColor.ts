...
const cssVar = (c: string): string => `var(--mimicus-${c})`;

export function resolveColor(color: string | undefined, defaultColor = ""): string {
  color ||= defaultColor;
  if (!color) return "";
...
export function colorMix(c1: string | undefined, c2: string, p: number): string {
