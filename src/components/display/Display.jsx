...
import { useDisplayBinding } from "../../display/useDisplayBinding.js";
...
function hashColor(s) { let h = 0; for (let i = 0; i < String(s).length; i++) h = String(s).charCodeAt(i) + ((h << 5) - h); return `hsl(${Math.abs(h) % 360} 55% 45%)`; }
...
/* ── Avatar ── */
export function Avatar({ src, alt, size = "medium", variant = "circular", children, className, style, ...rest }) {
...
export function AvatarGroup({ max = 5, total, spacing = "medium", children, className, style, ...rest }) {
...
/* ── Collapse / Accordion ── */
export function CollapsePanel({ panelKey, header, defaultOpen, children, className, ...rest }) {
  const autoId = useId();
...
export function Collapse({ accordion = false, defaultActiveKey, activeKey, children, className, style, ...rest }) {
  const ref = useRef(null);
...
/* ── Descriptions ── */
export function DescriptionsItem({ label, span = 1, children, className, ...rest }) {
  return (
...
export function Descriptions({ title, bordered = false, column = 3, size = "medium", children, className, style, ...rest }) {
  return (
    <div {...rest} className={cx("mimicus-descriptions", bordered && "mimicus-descriptions--bordered", `mimicus-descriptions--${size}`, className)} style={style}>
      ...
      <dl className="mimicus-descriptions__list" style={{ "--mimicus-desc-cols": column }}>
        {children}
      </dl>
    </div>
  );
...
/* ── Empty ── */
export function Empty({ description = "No data", image = "default", children, className, style, ...rest }) {
  return (
...
export function QRCode({ value = "", size = 128, bordered = true, className, style, ...rest }) {
  const ref = (el) => { if (el) paintQr(el, value, size); };
  return (
...
/* ── Segmented ── */
export function Segmented({ options = [], value, defaultValue, onChange, block, size = "medium", className, style, ...rest }) {
  const ref = useRef(null);
...
/* ── Statistic ── */
export function Statistic({ title, value, prefix, suffix, precision, className, style, ...rest }) {
...
/* ── Table ── */
export function Table({ columns = [], dataSource = [], bordered, size = "medium", sortable, pagination, className, style, children, ...rest }) {
  const ref = useRef(null);
  ...
  return (
    <table {...rest} ref={ref} className={cx("mimicus-table", bordered && "mimicus-table--bordered", `mimicus-table--${size}`, className)} style={style} data-mimicus-display={sortable ? "table" : undefined}>
      ...
      <tbody>{dataSource.map((row, ri) => (
        <tr key={row.key ?? ri}>{columns.map((col, ci) => <td key={col.key ?? ci}>{col.render ? col.render(row[col.dataIndex], row) : row[col.dataIndex]}</td>)}</tr>
      ))}</tbody>
    </table>
  );
}
...
/* ── Timeline ── */
export function TimelineItem({ label, color, dot, children, className, ...rest }) {
  return (
...
export function Timeline({ mode = "left", pending, children, className, style, ...rest }) {
  return (
...
/* ── Tooltip ── */
export function Tooltip({ title, placement = "top", arrow, children, className, ...rest }) {
  const ref = useRef(null);
...
/* ── Tour ── */
export function Tour({ steps = [], open = false, className, style, ...rest }) {
  const ref = useRef(null);
    ...
    <div className={cx("mimicus-tree__node", hasKids && "is-expanded")} data-mimicus-tree-node={key} style={{ paddingLeft: `${level * 1.1}rem` }}>
      ...
      {hasKids && (
        <div className="mimicus-tree__children" data-mimicus-tree-children>
          {node.children.map((c) => <TreeNode key={c.key ?? c.title} node={c} checkable={checkable} level={level + 1} />)}
        </div>
      )}
    </div>
  );
...
export function Tree({ treeData = [], checkable, defaultExpandAll, className, style, ...rest }) {
  const ref = useRef(null);
...
/* ── Calendar ── */
export function Calendar({ fullscreen, className, style, ...rest }) {
  const ref = useRef(null);
  const now = new Date();
...
/* ── List (MUI) ── */
export function ListItemText({ primary, secondary, className, ...rest }) {
  return (
...
export function ListItemIcon({ children, className, ...rest }) {
  return <span {...rest} className={cx("mimicus-list-item__icon", className)}>{children}</span>;
}

export function ListItemAvatar({ children, className, ...rest }) {
  return <span {...rest} className={cx("mimicus-list-item__avatar", className)}>{children}</span>;
}

export function ListItem({ button, secondaryAction, children, className, ...rest }) {
...
export function List({ dense, disableGutters, subheader, children, className, style, ...rest }) {
  return (
