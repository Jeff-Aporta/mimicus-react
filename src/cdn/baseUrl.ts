...
/** Base URL del folder cdn/ (termina en /). */
export function mimicusCdnBase(): string {
  ...
  const base = document.querySelector<HTMLBaseElement>("base")?.href || location.href;
  if (isDevHost || isGhPagesDemo || (globalThis as any).__MIMICUS_UI_DIST__) {
