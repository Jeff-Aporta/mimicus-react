import { emit, on, parseBool, parseNum, qs, qsa } from "../nav/core.ts";

type Cleanup = () => void;
// Algunos binders devuelven un arreglo de cleanups; `bindDisplayComponent` lo normaliza.
type BinderResult = Cleanup | Cleanup[];
// El nodo raíz lleva un cleanup adosado de forma dinámica; tipado laxo puntual.
type MimicusDisplayRoot = HTMLElement & { __mimicusDisplayCleanup?: Cleanup };

function cleanupPrev(root: HTMLElement, fn: Cleanup): void {
  const r = root as MimicusDisplayRoot;
  r.__mimicusDisplayCleanup?.();
  r.__mimicusDisplayCleanup = fn;
}

export function bindCarousel(root: HTMLElement): Cleanup {
  const track = qs<HTMLElement>("[data-mimicus-carousel-track]", root);
  const slides = qsa<HTMLElement>("[data-mimicus-carousel-slide]", root);
  const dots = qsa<HTMLElement>("[data-mimicus-carousel-dot]", root);
...
export function bindCollapse(root: HTMLElement): Cleanup {
  const accordion = parseBool(root.dataset.accordion);
  const panels = qsa<HTMLElement>("[data-mimicus-collapse-panel]", root);
  ...
  const setKeys = (keys: string[]) => {
    ...
    panels.forEach((p) => {
      ...
      const body = qs<HTMLElement>("[data-mimicus-collapse-body]", p);
...
export function bindTooltip(root: HTMLElement): Cleanup {
  const tip = qs<HTMLElement>("[data-mimicus-tooltip-popup]", root);
  const trigger = qs<HTMLElement>("[data-mimicus-tooltip-trigger]", root) ?? (root.firstElementChild as HTMLElement | null);
...
export function bindSegmented(root: HTMLElement): BinderResult {
  const btns = qsa<HTMLElement>("[data-mimicus-segmented]", root);
  const set = (v: string) => {
...
export function bindTree(root: HTMLElement): Cleanup {
  const checkable = parseBool(root.dataset.checkable);
  const nodes = qsa<HTMLElement>("[data-mimicus-tree-node]", root);

  const toggleExpand = (node: HTMLElement) => {
    const open = !node.classList.contains("is-expanded");
    node.classList.toggle("is-expanded", open);
    const child = qs<HTMLElement>("[data-mimicus-tree-children]", node);
  ...
  const cleanups = nodes.flatMap((node) => {
      ...
      const checkbox = qs<HTMLInputElement>("[data-mimicus-tree-check]", node);
      const outs: Cleanup[] = [];
      if (switcher) outs.push(on(switcher, "click", (e: Event) => { e.stopPropagation(); toggleExpand(node); }));
...
export function bindCalendar(root: HTMLElement): BinderResult {
  const grid = qs<HTMLElement>("[data-mimicus-calendar-grid]", root);
  const title = qs<HTMLElement>("[data-mimicus-calendar-title]", root);
...
export function bindTableSort(root: HTMLElement): Cleanup {
  const headers = qsa<HTMLElement>("[data-mimicus-table-sort]", root);
  if (!headers.length) return () => {};
  const tbody = qs<HTMLElement>("tbody", root);
...
export function bindTour(root: HTMLElement): BinderResult {
  const steps = qsa<HTMLElement>("[data-mimicus-tour-step]", root);
  const overlay = qs<HTMLElement>("[data-mimicus-tour-overlay]", root);
  const card = qs<HTMLElement>("[data-mimicus-tour-card]", root);
  const titleEl = qs<HTMLElement>("[data-mimicus-tour-title]", root);
  const descEl = qs<HTMLElement>("[data-mimicus-tour-desc]", root);
...
const BINDERS: Record<string, (root: HTMLElement) => BinderResult> = {
...
export function bindDisplayComponent(root: HTMLElement): Cleanup {
  const r = root as MimicusDisplayRoot;
  const type = r?.dataset?.mimicusDisplay;
  cleanupPrev(r, () => {});
  const cleanup: BinderResult = (type ? BINDERS[type]?.(r) : undefined) ?? (() => {});
  const fn: Cleanup = Array.isArray(cleanup) ? () => cleanup.forEach((c) => c?.()) : cleanup;
  r.__mimicusDisplayCleanup = fn;
  return () => { fn(); delete r.__mimicusDisplayCleanup; };
}

export function mountDisplay(scope: ParentNode | HTMLElement = document): Cleanup {
  const roots = scope === document
    ? qsa<HTMLElement>("[data-mimicus-display]", document)
    : [scope as HTMLElement].filter((el) => el?.dataset?.mimicusDisplay);
