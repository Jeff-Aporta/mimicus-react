/**
 * LoginFormFields — isla: contapyme
 * repo: Jeff-Aporta/mimicus-react · src/contapyme/components/LoginFormFields.tsx
 * Campos y acciones del formulario de login (usuario, contraseña, empresa, recordarme).
 */
import type { Dispatch, SetStateAction } from "react";
import { Button } from "../../components/Button.tsx";
import { Checkbox } from "../../components/forms/Forms.tsx";
import { FormItem, Input, Select } from "../../components/forms/Forms.tsx";
...
// Forma de tercero de multiempresa (proviene del backend, heterogénea); `any` puntual.
type Tercero = { itercero?: string | number; nombre?: string; razonsocial?: string; [key: string]: any };
...
export interface LoginFormFieldsProps {
  ...
  onEnter?: () => void;
  terceros?: Tercero[];
  selectedItercero?: string;
  setSelectedItercero: (v: string) => void;
}
    ...
    <Stack spacing="0.75rem" className="mimicus-login-form">
      ...
      <FormItem label="Usuario" help={CONTAPYME_LOGIN_ID_HELPER}>
        <Input
          ...
          onKeyDown={(e: any) => { if (e.key === "Enter" && onEnter) { e.preventDefault(); onEnter(); } }}
        />
      </FormItem>
...
export function LoginFormActions({ busy, canSubmit, onCancel, onSubmit, showCancel = true }: LoginFormActionsProps) {
  return (
