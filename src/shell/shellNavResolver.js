...
export function sortedCategories(ctx) {
...
export function sectionMeta(ctx, sectionId) {
  return ctx.catalog?.sections?.[sectionId] ?? ctx.catalog?.categories?.[sectionId] ?? {};
}

export function sectionColorFor(ctx, categoryId) {
...
export function resolveCategoryTabDescriptors(ctx) {
...
export function resolveCatalogDemoTabDescriptors(ctx) {
  const { route, catalogItems } = ctx;
  if (!route?.category) return [];
  const color = sectionColorFor(ctx, route.category);
  return catalogItems
    .filter((it) => (it.section ?? it.category) === route.category)
    .map((it) => ({
      id: it.slug,
...
export function normalizeShellNavRows(shellConfig) {
...
export function evaluateNavWhen(when, ctx) {
  const w = when ?? "always";
...
export function resolveNavRowTabs(row, ctx) {
  const tabs = [];
  for (const child of row.children ?? []) tabs.push(...resolveNavNode(child, ctx));
  return tabs;
}

export function resolveNavRowValue(row, ctx) {
...
export function resolveShellNavigation(shellConfig, ctx) {
  return {
...
export function dispatchShellNav(tabId, ctx, shellConfig) {
