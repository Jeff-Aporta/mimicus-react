import { ensureCodeMirrorCss } from "./lazyLoad.js";

export function readMimicusLuminance() {
...
export function resolveCodeMirrorTheme(luminance = readMimicusLuminance()) {
...
export function syncCmBoundedSize(cm, maxHeight, host, minHeight) {
...
export function mountCodeMirror(host, opts = {}) {
  ...
  const extraKeys = readOnly
    ? { "Ctrl-A": (cm) => cm.execCommand("selectAll"), "Cmd-A": (cm) => cm.execCommand("selectAll"), ...(opts.extraKeys || {}) }
    : { Tab: (ed) => ed.replaceSelection("  ", "end"), "Ctrl-A": (cm) => cm.execCommand("selectAll"), "Cmd-A": (cm) => cm.execCommand("selectAll"), ...(opts.extraKeys || {}) };
...
export async function copyEditorText(text) {
  const s = String(text ?? "");
  if (!s) return false;
  try {
