...
import { resolveColor } from "@jeff-aporta/mimicus-react";
...
export function Icon({ icon, className, style }) {
  if (!icon) return null;
  return <iconify-icon icon={icon} class={className} style={style} />;
}
...
export function CatalogHome({
  ...
  const heroTitle = filterSection ? sectionLabel(filterSection) : (overviewMeta.title ?? "Components Overview");
  const heroLead = filterSection ? sectionDescription(filterSection) : (overviewMeta.lead ?? MIMICUS_UI.catalogLead);
      ...
      <div className="catalog-overview__main catalog-home">
        {showHero && (
          <header className="catalog-overview__hero pg-vt-hero">
            <p className="catalog-home__kicker mimicus-ui-brand">{overviewMeta.kicker ?? MIMICUS_UI.name}</p>
